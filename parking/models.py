...
class UserManager(BaseUserManager):
    def create_user(self, email, username, password=None, national_id=None, **extra_fields):
        ...
        # ✅ أهو الكود المهم اللي يمنع الأيرور
        default_group, _ = Group.objects.get_or_create(name='User')
        user.groups.set([default_group])  # الصح بدل من user.groups = [...]
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    username = models.CharField(max_length=255, null=False)  # ✅ هنا كان فيه غلط
    email = models.EmailField(unique=True, null=False)
    profile_picture = models.ImageField(upload_to='',  null=True, blank=True)
    phone_number = models.CharField(max_length=15,  null=False ,  unique=True, default='01111111111')
    password = models.CharField(max_length=255, null=False )
    DOB = models.DateField(default=now, null=False)
    gender = models.CharField(max_length=10, null=False, choices=[('Male', 'Male'), ('Female', 'Female')],default='Male')
    national_id = models.CharField(max_length=20, null=False, default='11111111111111')
    nationality = models.CharField(max_length=3, null=False,choices=Nationality.choices,default=Nationality.OTHER)
    subscription_type  = models.CharField(max_length=20, choices=[('standard', 'standard'), ('VIP', 'VIP')],default='standard', null=False)
    license_id = models.ImageField(upload_to='', blank=True, null=False)
    Registration_Date = models.DateTimeField(default=now, null=False)
...
class Vehicle(models.Model):
    ...
    vehicle_type = models.CharField(max_length=20, null=False)  # e.g., Car, Bike, Truck
    user = models.ForeignKey('parking.User',on_delete=models.CASCADE)
    car_model = models.CharField(max_length=20, null=False)
    vehicle_color = models.CharField(max_length=20,default='no-color', null=False)
